...
class Integrator:
    ...
    def leapfrog(self, particles:list, galaxy:Galaxy, dt:float, steps:int) -> tuple[np.ndarray]:
        ...
        Returns:
            ...
            energies_BH (np.ndarray or None): Total energies of the perturbers at each step [steps] or None.
            positions_BH (np.ndarray or None): Positions of the perturbers at each step [steps, 3] or None.
            velocities_BH (np.ndarray or None): Velocities of the perturbers at each step [steps, 3] or None.
        """
        ...
        if hasattr(galaxy, 'perturbers'):
            energies_BH = np.zeros((len(galaxy.perturbers), steps))    # Array to store perturbers energies
        else:
            energies_BH = None
        ...
        # If there is P perturbers, initialize their positions and velocities
        if hasattr(galaxy, 'perturbers'):
            perturbers = galaxy.perturbers
            pos_BH = np.copy([pert.position for pert in perturbers])   # [P, 3]
            vel_BH = np.copy([pert.velocity for pert in perturbers])   # [P, 3]

            # Store the perturbers' trajectories
            positions_BH  = np.zeros((len(perturbers), steps, 3))      # [P, N, 3]
            velocities_BH = np.zeros((len(perturbers), steps, 3))      # [P, N, 3]

            # Calculate initial acceleration for the perturbers [P, 3]
            acc_BH = np.array([pert.acceleration(pos_BH[perturbers.index(pert)]) for pert in perturbers])

            # Initialize half-step velocity for the perturber
            vel_BH_half = vel_BH + 0.5 * dt * acc_BH                   # [P, 3]
            # Calculate initial accelerations for stars using the perturbers' positions
            acc = galaxy.acceleration(pos, perturbers_pos=pos_BH)      # [N, 3]
        ...
        for i in range(steps):
            ...
            # Update perturbers' position if present
            if positions_BH is not None:
                pos_BH += dt * vel_BH_half                             # [P, 3]
                positions_BH[:,i] = pos_BH

                # Update galaxy's perturbers position
                for index, pert in enumerate(perturbers):
                    pert.position = pos_BH[index]

            # --- Compute new accelerations based on updated positions ---
            if hasattr(galaxy, 'perturbers'):
                # Compute accelerations using the updated perturbers position
                acc_new = galaxy.acceleration(pos, perturbers_pos=pos_BH)                                         # [N, 3]
                acc_BH_new = np.array([pert.acceleration(pos_BH[perturbers.index(pert)]) for pert in perturbers]) # [P, 3]
            else:
            ...
            # Update perturbers' velocity if present
            if positions_BH is not None:
            ...
            # Add potential energy due to the perturbers
            if hasattr(galaxy, 'perturbers'):
                for pert in perturbers:
                    delta_r = pos_BH[perturbers.index(pert)] - pos  # [N, 3]
                    r = np.linalg.norm(delta_r, axis=1)  # [N]
                    with np.errstate(divide='ignore', invalid='ignore'):
                        # Compute PE = -G * m_p * m_i / r
                        potential_energy_pert = -galaxy.G * pert.M * masses / r  # [N]
            ...
            # --- Compute and Store Perturbers' Energy ---
            if hasattr(galaxy, 'perturbers') and energies_BH is not None:
                for index, pert in enumerate(perturbers):
                    ...
                    PE_BH_galaxy = galaxy.potential(R_BH, pos_BH[index, 2])

                    # --- Potential Energy due to Stars ---
                    # Compute the gravitational potential at the perturbers' position due to all stars
                    ...
                    # Total Potential Energy of Perturbers
                    PE_BH = PE_BH_galaxy + potential_energy_stars

                    # Total Energy of Perturbers
                    energies_BH[index, i] = KE_BH + pert.M * PE_BH
        ...
        # Return positions and velocities of stars and perturbers, along with energies
    ...
    def rk4(self, particles:list, galaxy:Galaxy, dt:float, steps:int) -> tuple[np.ndarray]:
        """
        Runge-Kutta 4th order integrator for orbit simulation, including the perturbers.
        ...
        energies = np.zeros((steps, N))
        angular_momenta = np.zeros((steps, N))
        energies_BH = np.zeros(steps)  # Array to store perturbers' energy
        positions_BH = None
        velocities_BH = None
