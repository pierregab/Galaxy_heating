...
class Integrator:
    ...
    def leapfrog(self, particles: list, galaxy: Galaxy, dt: float, steps: int) -> tuple:
        ...
        if hasattr(galaxy, 'perturbers') and len(galaxy.perturbers):
            ...
            angular_momenta_BH = np.zeros((steps, P))  # Correct shape
        ...
        if pos_BH is not None:
            ...
            if P > 1:
                ...
                acc_BH_perturbers = galaxy.G * np.sum(
                    perturbers_mass[np.newaxis, 1:] * delta_r_perturbers / softened_r_perturbers[:, :, np.newaxis]**3,
                    axis=1
                )  # [P,3]
            else:
            ...
            if pos_BH is not None:
                ...
                if P > 1:
                    ...
                    acc_BH_perturbers_new = galaxy.G * np.sum(
                        perturbers_mass[np.newaxis, 1:] * delta_r_perturbers_new / softened_r_perturbers_new[:, :, np.newaxis]**3,
                        axis=1
                    )  # [P,3]
                else:
